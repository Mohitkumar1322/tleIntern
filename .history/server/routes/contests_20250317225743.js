...
import ContestYT from "../models/ContestYT.js";
  ...
  try {
    ...
    router.post("/add", async (req, res) => {
        try {
            ...
            const contest = new Contest({ name, youtubeLink });
            await contest.save();
            res.status(201).json({ message: "Contest link added successfully!" });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    
    // Get all contest links
